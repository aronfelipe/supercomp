...
double knapSack(int value_of_portfolio, vector<stock> stocks, vector<stock>& used, vector<stock>& best) {
    ...
    if(stocks.empty() || value_of_portfolio == 0) {
        ...
        double actual_value = 0.0, best_value = 0.0, bound = 0.0;
        for(auto& el : used){
        ...
        for(auto& el: stocks) {
            bound = bound + el.return_of_batch - el.tax_of_batch;
        }

        if (actual_value+bound<=best_value)  
           return 0;

        if(stocks[0].value_of_batch <= value_of_portfolio){
            used.push_back(stocks[0]);
            stocks.erase(stocks.begin());
            value = stocks[0].return_of_batch;
            com_i = knapSack(value_of_portfolio - stocks[0].value_of_batch, stocks, used, best);
        }
